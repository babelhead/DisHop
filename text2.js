...
But now the question is how to limit this to just one action without requiring a click...*/
...
function launchAnim(){
    ...
    function onTick(){
        ...
        // if (texte.textContent == "making the text appear letter by letter, each sliding up and fading in to its final place and state one after the other. I realized only when testing on a longer paragraph that this method makes the spaces disappear between the words, so it all comes very hard to read. It happens in the video as well, by the way, but the guy ignores it (or doesn’t even see – with his capitalized two-word example, it’s less bothering, but still…") {
        // - why would something like this not work as a condition for completion? tried with innerHTML, same story.. I don't understand why the initial solution (below) stopped working, it did before... (now it does again... a mystery...)
    ...
    function complete(){
        ...
        newPara2.textContent = "Also, it seems to slow down the page a lot, the background image takes ages to load, and now that I added a paragraph before this one in the same article, the animation seems to have slowed down too, even though I actually speeded it up in the script. (Now I've reduced the size of the background image and it seems to load better again, if still a bit slower than it used to, and the animation goes with a good speed too.) I also had to figure out how to launch the animation only when the reader gets to this article and not directly on the loading of the site, and to add these other paragraphs in the same article once the animation is complete. The work of refinement is never finished, there is always more to do, things you can only see when you launch a completed version… It’s also a bit like a Rubik cube: you move or change one element and it breaks the whole structure, so you need to rethink the whole – or at least a good part of the immediate context, and make sure there are no repercussions further.";
